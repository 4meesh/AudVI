...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function setupExpandingCirclesPreloader() {
    ...
    function animate(timestamp) {
      ...
      for (let c = 0; c < circleCount; c++) {
        ...
        ctx.strokeStyle = `rgba(16, 255, 193, ${opacity * 0.2})`;
        ctx.lineWidth = 1;
        ctx.stroke();
        for (let i = 0; i < dotCount; i++) {
          ...
          ctx.lineTo(x, y);
          ctx.strokeStyle = `rgba(16, 255, 193, ${opacity * 0.1})`;
          ctx.lineWidth = 1;
          ctx.stroke();
          ctx.beginPath();
          ctx.arc(x, y, size, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(16, 255, 193, ${opacity * 0.9})`;
          ctx.fill();
        }
      }
  ...
  function attachAudioPlayerEvents(audioElement) {
  ...
  function createNewAudioElement() {
    ...
    attachAudioPlayerEvents(newAudioElement);
  ...
  function drawCircularVisualizer() {
    ...
    circularCtx.fillStyle = "rgba(16, 255, 193, 0.05)";
    circularCtx.fill();
    const numRings = 3;
    for (let ring = 0; ring < numRings; ring++) {
      ...
      if (ring === 0) {
        ...
        gradient.addColorStop(0, `rgba(16, 255, 193, ${opacity})`);
        gradient.addColorStop(1, `rgba(16, 255, 193, ${opacity * 0.7})`);
      } else if (ring === 1) {
        gradient = circularCtx.createRadialGradient(
          centerX,
          centerY,
      ...
      circularCtx.shadowColor = "rgba(16, 255, 193, 0.7)";
    }
  ...
  function drawSpectrumAnalyzer() {
    ...
    for (let i = 0; i < 256; i++) {
      ...
      spectrumCtx.fillStyle = `rgba(16, 255, 193, 0.7)`;
      spectrumCtx.fillRect(x, height - barHeight, barWidth - 1, barHeight);
      x += barWidth;
    }
    spectrumCtx.strokeStyle = "rgba(16, 255, 193, 0.2)";
  ...
  function updateAudioWave() {
    ...
    wave.style.borderColor = `rgba(16, 255, 193, ${
  ...
  function calculateAudioMetrics() {
    ...
    if (stabilityValue < 40) {
      document.getElementById("status-indicator").style.color = "#10ffc1";
  ...
  function updateTimestamp() {
    ...
    document.getElementById(
      "timestamp"
    ).innerHTML = `TIME: ${hours}:${minutes}:${seconds}<br />ameesh mohammed`;
  }
  ...
  function drawWaveform() {
    ...
    waveformCtx.fillStyle = "rgba(16, 255, 193, 0.2)";
    waveformCtx.fillRect(0, 0, width, height);
    if (audioAnalyser) {
      audioAnalyser.getByteTimeDomainData(audioData);
      waveformCtx.beginPath();
      waveformCtx.strokeStyle = "rgba(16, 255, 193, 0.8)";
  ...
  function initThreeJS() {
    ...
    scene.fog = new THREE.FogExp2(0x10ffc1, 0.05);
    ...
    renderer.setClearColor(0x10ffc1, 0);
    ...
    const ambientLight = new THREE.AmbientLight(0x10ffc1, 1.5);
    scene.add(ambientLight);
    const directionalLight = new THREE.DirectionalLight(0x10ffc1, 1.5);
    directionalLight.position.set(1, 1, 1);
    scene.add(directionalLight);
    const pointLight1 = new THREE.PointLight(0x10ffc1, 1, 10);
    pointLight1.position.set(2, 2, 2);
    scene.add(pointLight1);
    const pointLight2 = new THREE.PointLight(0x10ffc1, 1, 10);
  ...
  function createBackgroundParticles() {
    ...
    const color1 = new THREE.Color(0x10ffc1);
    const color2 = new THREE.Color(0x10ffc1);
    const color3 = new THREE.Color(0x10ffc1);
  ...
  function createAnomalyObject() {
    ...
    const outerMaterial = new THREE.ShaderMaterial({
      uniforms: {
        ...
        color: {
          value: new THREE.Color(0x10ffc1),
        },
        audioLevel: {
          value: 0,
        },
  ...
  reactivitySlider.addEventListener("input", function () {
    ...
    document.getElementById("reactivity-value").textContent =
      audioReactivity.toFixed(1);
  });
  sensitivitySlider.addEventListener("input", function () {
    audioSensitivity = parseFloat(this.value);
    document.getElementById("sensitivity-value").textContent =
      audioSensitivity.toString();
  });
  ...
  // Make the new audio controls panel draggable
  makePanelDraggable(
    document.getElementById("audio-controls-panel"),
    document.getElementById("audio-controls-handle")
  );

  // Mini audio controls wiring
  const miniVolume = document.getElementById('mini-volume');
  const miniBass = document.getElementById('mini-bass');
  const miniEcho = document.getElementById('mini-echo');
  const extraEcho = document.getElementById('extra-echo');

  let bassFilter = null;
  let echoDelay = null;
  let echoGain = null;
  let extraEchoDelay = null;
  let extraEchoGain = null;

  function setupMiniAudioEffects(audioCtx, sourceNode) {
    // Disconnect all previous connections
    try {
      if (bassFilter) bassFilter.disconnect();
      if (echoDelay) echoDelay.disconnect();
      if (echoGain) echoGain.disconnect();
      if (extraEchoDelay) extraEchoDelay.disconnect();
      if (extraEchoGain) extraEchoGain.disconnect();
      // Disconnect source but preserve analyser connection to destination
    ...
    bassFilter.gain.value = parseFloat(miniBass.value);

    // Main Echo
    echoDelay = audioCtx.createDelay();
    echoDelay.delayTime.value = parseFloat(miniEcho.value) * 0.5;
    echoGain = audioCtx.createGain();
    echoGain.gain.value = parseFloat(miniEcho.value) * 0.7;

    // Extra Echo
    extraEchoDelay = audioCtx.createDelay();
    extraEchoDelay.delayTime.value = parseFloat(extraEcho.value) * 1.0;
    extraEchoGain = audioCtx.createGain();
    extraEchoGain.gain.value = parseFloat(extraEcho.value) * 0.7;

    // Connect: source -> bass -> analyser (for visualization) -> destination
    // analyser is already connected to destination from initAudio()
    sourceNode.connect(bassFilter);
    bassFilter.connect(audioAnalyser);
    
    // Main echo path (parallel to analyser)
    bassFilter.connect(echoDelay);
    echoDelay.connect(echoGain);
    echoGain.connect(audioCtx.destination);
    
    // Extra echo path (parallel to analyser)
    bassFilter.connect(extraEchoDelay);
    extraEchoDelay.connect(extraEchoGain);
    extraEchoGain.connect(audioCtx.destination);

    console.log('Audio FX chain rebuilt:', {
      bass: bassFilter.gain.value,
      echo: echoDelay.delayTime.value,
      extraEcho: extraEchoDelay.delayTime.value
    });
  }

  function updateMiniAudioEffects() {
    if (bassFilter) bassFilter.gain.value = parseFloat(miniBass.value);
    if (echoDelay) echoDelay.delayTime.value = parseFloat(miniEcho.value) * 0.5;
    if (echoGain) echoGain.gain.value = parseFloat(miniEcho.value) * 0.7;
    if (extraEchoDelay) extraEchoDelay.delayTime.value = parseFloat(extraEcho.value) * 1.0;
    if (extraEchoGain) extraEchoGain.gain.value = parseFloat(extraEcho.value) * 0.7;
    console.log('Audio FX updated:', {
      bass: bassFilter ? bassFilter.gain.value : null,
      echo: echoDelay ? echoDelay.delayTime.value : null,
      extraEcho: extraEchoDelay ? extraEchoDelay.delayTime.value : null
    });
  }
  ...
  if (miniVolume) {
    miniVolume.addEventListener('input', function() {
      ...
      audioEls.forEach(a => { a.volume = parseFloat(this.value); });
    });
  }
  [miniBass, miniEcho, extraEcho].forEach(ctrl => {
    if (ctrl) ctrl.addEventListener('input', updateMiniAudioEffects);
  });

  // Integrate with existing audio context setup
  // Patch into setupAudioSource and initAudioFile/loadAudioFromURL
  const origSetupAudioSource = setupAudioSource;
  setupAudioSource = function(audioElement) {
    const result = origSetupAudioSource(audioElement);
    if (result && audioContext && audioSource) {
      setupMiniAudioEffects(audioContext, audioSource);
    }
    return result;
  };
});
